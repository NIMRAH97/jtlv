 ...
 * 
 */
public class BugyParallelGRGame implements Game {
	...
	/**
	 * for this implementation, pool_size must be set to 10. <br>
	 ...
	 */
	public BugyParallelGRGame(ModuleWithWeakFairness env, ModuleWithWeakFairness sys)
			throws GameException {
	...
	private BDD calculate_win() {
		...
		int cy = 0;
		InnerX[] allNu = new InnerX[env.justiceNum()];
		Semaphore[] work = new Semaphore[env.justiceNum()];
		Semaphore join = new Semaphore(0, true);
		for (int i = 0; i < env.justiceNum(); i++) {
			work[i] = new Semaphore(0, true);
			allNu[i] = new InnerX(work[i], join);
		}
		for (int i = 0; i < env.justiceNum(); i++) {
			allNu[i].start();
		}
		join.acquireUninterruptibly(env.justiceNum());
		...
		for (iterZ = new FixPoint<BDD>(); iterZ.advance(z);) {
			for (int j = 0; j < sys.justiceNum(); j++) {
				...
				for (iterY = new FixPoint<BDD>(); iterY.advance(y);) {
					...
					for (int i = 0; i < env.justiceNum(); i++) {
						allNu[i].reinit(j, i, cy, z.id(), start.id());
					}
					for (int i = 0; i < env.justiceNum(); i++) {
						work[i].release(1);
					}
					join.acquireUninterruptibly(env.justiceNum());
					for (int i = 0; i < env.justiceNum(); i++) {
						x_mem[j][i][cy] = allNu[i].getResultInBaseForm().id();
					}

					for (int i = 0; i < env.justiceNum(); i++) {
						y = y.id().or(x_mem[j][i][cy]);
					}
	...
	public class InnerX extends JTLVThread {
		...
		public InnerX(Semaphore myWorkingSem, Semaphore myJoinSem) {
			alive = true;
			work = myWorkingSem;
			join = myJoinSem;
		...
		public void reinit(int j, int i, int cy, BDD Z, BDD start) {
			this.j = j;
			this.i = i;
			this.cy = cy;
			...
			if (j == 0 & i == 1 & cy == 1)
				System.out.println("in the bug construction");
		}

		public void run() {
			while (true) {
				join.release(1);
				work.acquireUninterruptibly(1);
				if (!alive)
					break;
				if (j == 0 & i == 1 & cy == 1)
		...
		public BDD getResultInBaseForm() {
			BDD res = adjustBDDToBase(X.id()).id();
			// X.getFactory().done();
			return res;
		}
