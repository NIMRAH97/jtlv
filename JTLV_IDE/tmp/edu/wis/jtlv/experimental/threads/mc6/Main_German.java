...
public class Main_German {
	...
	public static void usage(String[] args) {
		...
		try {
			...
			String mName = Main_German.class.getCanonicalName();
	...
	public static void do_parallel_trans(SMVModule main, int N, int threadNum,
		...
		SplitInvThread[] threads;
		threads = new SplitInvThread[N];

		// constructing the threads
		threads[0] = new HomeThread(0, N, main, verbose);
		//threads[0].setPriority(Thread.MAX_PRIORITY);
		// without resizing during the algorithm for German 5...
		Env.copyBDDIntoManager(threads[0].getDedicatedBDDManagerIdx(),
				Env.TRUE()).getFactory().setNodeTableSize(9000000);
		// without resizing during the algorithm for German 6...
		// Env.copyBDDIntoManager(threads[0].getDedicatedBDDManagerIdx(),
		// Env.TRUE()).getFactory().setNodeTableSize(35000000);

		for (int i = 1; i < N; i++) {
			threads[i] = new ClientThread(i, N, main, verbose);
			//threads[i].setPriority(Thread.MIN_PRIORITY);
		}

		// constructing the semaphores (only for sanity check):
		Semaphore semMain = new Semaphore(0, true);
		Semaphore[] semThread = new Semaphore[N];
		for (int i = 0; i < N; i++) {
			semThread[i] = new Semaphore(0, true);
		}

		for (int i = 0; i < N; i++) {
			threads[i].setOtherThreads(threads);
			threads[i].setSemaphores(semMain, semThread[i]);
		}
		...
		// Ariel: maybe this should be done with wait so to be more efficient?
		// (so we never execute this thread in cpu untill turned on)
		// System.out.println("Main is waiting on semaphore");
		semMain.acquireUninterruptibly(N);
		long time = System.currentTimeMillis();
		System.out.println("Overall time parallel - " + (time - start));

		// obtain newthread from each of the threads
		BDD foundStates = Env.TRUE();
		for (int i = 0; i < N; i++) {
			foundStates = foundStates.and(threads[i].getNewTheta());
		}

		// release threads
		for (int i = 0; i < N; i++) {
			semThread[i].release(1);
		}
