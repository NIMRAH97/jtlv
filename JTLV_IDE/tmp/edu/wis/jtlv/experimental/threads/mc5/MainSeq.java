...
public class MainSeq {
	...
	public static void usage(String[] args) {
		...
		try {
			...
			String mName = MainSeq.class.getCanonicalName();
	...
	public static SMVModule loadMainModule(String input_file) {
		...
		//System.setProperty("bdd_table_size", "200000");
		//System.setProperty("bdd_cache_size", "200000");
		System.setProperty("bdd_table_size", "20000");
		System.setProperty("bdd_cache_size", "20000");
		Env.resetEnv();
		Env.beQuiet();
	...
	public static void do_mc_trans(SMVModule main, int N) {
		...
		for (int i = 1; i < N; i++) {
			...
			for (int j = 1; j < N; j++) {
				...
				ModuleBDDField temp_home1 = clients[0].getVar("invalidate_list[" + j + "]",
						false);
...
//System.out.println("tempTrans0:\n" + Env.toNiceString(main, tempTrans0));
	...
	public static void do_sequential_trans(SMVModule main, int N) {
		...
		//just taking care of the idle step
		for (int i = 0; i < N; i++) {
			// the idle trans
		...
		BDD[] idleFromPObtainByHome = new BDD[N];	
		for (int i = 1; i < N; i++) {
			varsHomeToQuantifyForP[i] = primeUnprimeLocalVars[0];
			idleFromPObtainByHome[i] = idleTrans[0];
			for (int j = 1; j < N; j++) {
				if (j != i) {
					...
					idleFromPObtainByHome[i] = idleFromPObtainByHome[i].and(h_cha1[j].getDomain().buildEquals(
							h_cha1[j].prime().getDomain()));
					idleFromPObtainByHome[i] = idleFromPObtainByHome[i].and(h_cha2[j].getDomain().buildEquals(
							h_cha2[j].prime().getDomain()));	
					idleFromPObtainByHome[i] = idleFromPObtainByHome[i].and(h_cha3[j].getDomain().buildEquals(
							h_cha3[j].prime().getDomain()));					
				}
			}
		}
...
//System.out.println("counter = " + counter);
			...
			for (int j = 1; j < N; j++) {
				...
				.or(jGlobalTransToBeUsedByHome[j].and(idleFromPObtainByHome[j]));
			}
			...
			for (int i = 1; i < N; i++) {
				...
				for (int j = 1; j < N; j++) {
					if (j != i) {
						...
						.or(jGlobalTransToBeUsedByProc[j].and(idleByOtherP[i]));
					}
				}
	...
	public static void do_parallel_trans(SMVModule main, int N, int threadNum,
		...
		// ///////// === FINISH initial settings
/*		Channel[] outputChannels = new Channel[threadNum];
		//output channels:
		for (int i = 0; i < threadNum; i++) {
			outputChannels[i] = a.new Channel(a);
		}
*/		
		...
		for (int i = 0; i < threadNum; i++) {
			...
			threads[i] = new MCThreadTrans(i, clients[i], idleTrans[i],
					vars[i], initCond[i], sync, sem[i]);
			// threads[i].start();
		}
		...
		do {
			...
			System.out.println("main in step 1");
			// getting the global transition of home
			for (int i = 1; i < threadNum; i++) {
				homeGlobalTransToBeUsedByProc[i] = (globalTrans[0])
				.exist(varsHomeToQuantifyForP[i]);
			}
			System.out.println("main in step 2");
			// getting the global transition of each process
			for (int i = 1; i < threadNum; i++) {
				jGlobalTransToBeUsedByHome[i] = (globalTrans[i])
				.exist(primeUnprimeLocalVars[i]);
				jGlobalTransToBeUsedByProc[i] = (globalTrans[i])
				.exist(varsToQuantifyForP[i]);
			}
			System.out.println("main in step 3");
			...
			// extTrans[0] =
			// extTrans[0].or(othersGlobalTransForHome.and(idleTrans[0]));
			threads[0].setOthersGlobalTrans(othersGlobalTransForHome
					.and(idleTrans[0]));
			System.out.println("main release " + 0);
			sem[0].release(1);
			...
			for (int i = 1; i < N; i++) {
				...
				// extTrans[i] =
				// extTrans[i].or(othersGlobalTrans.and(idleTrans[i
				// ]).and(idleByOtherP[i]));
				threads[i].setOthersGlobalTrans(othersGlobalTrans.and(
						idleTrans[i]).and(idleByOtherP[i]));
				// let i continue
	...
	static class MCThreadTrans extends JTLVThread {
		...
		public MCThreadTrans(int _id, SMVModule clients, BDD _idleStep,
				BDDVarSet _vars, BDD _initCond, Semaphore _sync, Semaphore _sem) {
			False = adjustBDDToManager(Env.FALSE());
			True = adjustBDDToManager(Env.TRUE());
			globalTrans = False;
			othersGlobalTrans = False;
			trans = assignTransToDedicatedManager(clients);
			extTrans = assignTransToDedicatedManager(clients);
			this.id = _id;
			...
			// extTrans = trans;
			// newTheta = oldTheta;
			newTheta = adjustBDDToManager(_initCond);
		}
		...
		public void run() {
			computeNewThetas();
			while (alive) {
				System.out.println("id=" + id + "in while");
				if (foundNewThetas)
					computeGlobalTrans();
				else
					globalTrans = False;
				sync.release(1);
				System.out.println("id=" + id + "release sync");
		...
		public void computeGlobalTrans() {
			globalTrans = (trans.and(oldTheta).and(Env.prime(newTheta)));
		}
		...
		// maybe we can make sure that this thread is doing
		// the adjusting operation -- by having a channel
		public void setOthersGlobalTrans(BDD _othersGlobalTrans) {
			...
			extTrans = extTrans.or(othersGlobalTrans);
		}
