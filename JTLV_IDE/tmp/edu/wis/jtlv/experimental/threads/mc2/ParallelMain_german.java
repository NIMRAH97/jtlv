...
public class ParallelMain_german extends JTLVThread {
	...
	public ParallelMain_german(SMVModule _main, int _N, int _threadNum,
		...
		for (int i = 1; i < N; i++) {
			for (int j = 1; j < N; j++) {
				if (j != i) {
					varsHomeToQuantifyForP[i] = local_h_cha1[j];
	...
	public void run() {
		...
		// ParallelMCMain_german2.main_german.initCond));
		// System.out.println(Env.toNiceString(main, initCond));
		...
		// isn't working
		// oldTheta = adjustBDDToManager(client.initial());

		// compute theta of home
		...
		do {
			...
			newTheta = newAllSucc(oldTheta, extTrans, vars);
			foundNewThetas = !(newTheta.equals(oldTheta));
			System.out.println("main computing global trans ");
			if (!foundNewThetas) {
				globalTrans = False;
				System.out.println("---> which is False");
			} else {
				globalTrans = (trans.and(oldTheta).and(Env.prime(newTheta)));
				System.out.println("---> which is NOT False");

				System.out.println(" Home's globalTrans:\n "
						+ Env.toNiceString(main, adjustBDDToBase(globalTrans)));
			...
			for (int i = 1; i < N; i++) {
				// first take home's
				if (globalTrans.equals(False)) {
					System.out.println("main writing False on channel " + i);
					outputChannel[i].writeBDD(False);
				} else {
					System.out.println("main writing globalTrans on channel "
							+ i);
			...
			System.out.println("main: foundNewThetas is " + foundNewThetas);

			if (foundNewThetas) {
				for (int i = 1; i < N; i++) {
					System.out.println("main is obtaining global trans from "
							+ i);
					BDD temp = adjustBDDToManager(threads[i].getGlobalTrans());
					extTrans = extTrans.or(temp);
				}
