package edu.wis.jtlv.experimental.threads.mc2;
...
public class MCThreadTrans extends JTLVThread {
	...
	public MCThreadTrans(int _id, int _N, SMVModule _main, // BDD _idleStep,
			...
			Semaphore _sync, Semaphore _sem, Channel _inputChannelFromHome) {
		...
		while (alive) {
			...
			if (foundNewThetas) {
				System.out.println("id=" + id
						+ " is computing globalTrans -- NOT False");
				computeGlobalTrans(); // this is also globalTrans for home
				globalTransForP = globalTrans.exist(varsToQuantifyForP);
				if (id == 1) {
					System.out.println(" ====>  id=1 computed:\n "
							+ Env.toNiceString(main,
									adjustBDDToBase(globalTransForP)));
				}

			} else {
				System.out.println("id=" + id
						+ " is computing globalTrans -- FALSE");
				globalTrans = False;
				globalTransForP = False;
			}
			// first writing to all other P
			System.out.println("id=" + id
					+ " is putting globalTrans on outputchannels");
			for (int i = 1; i < N; i++) {
				if (i != id)
					outputChannels[i].writeBDD(globalTransForP);
			}
			// next letting main know so he could get the global trans for home
			sync.release(1);
			System.out.println("id=" + id + " released sync!");
			sem.acquireUninterruptibly(1);
			// MUST BE THAT SOMEONE did getGlobalTrans()
			// and then setOthersGlobalTrans(BDD _othersGlobalTrans)
			System.out.println("id=" + id + " is obtatining new trans");
			obtainGlobalTrans();
			if (alive)
				computeNewThetas();
		}
	...
	private void obtainGlobalTrans() {
		...
		/*
		 * BDD reading = inputChannelFromHome.read(); if(reading == null){
		 * System.out.println("id=" + id + " got null!!!!!!!!!!!"); } else
		 * System.out.println("id=" + id + " didn't get null%%%");
		 * 
		 * 
		 * BDD fromHome = adjustBDDToManager(reading);
		 */

		BDD fromHome = adjustBDDToManager(inputChannelFromHome.read());
		othersGlobalTrans = (fromHome).and(idleTrans);

		if (id == 1) {
			System.out.println("  ------>  id=1 got FROM HOME:\n "
					+ Env.toNiceString(main, adjustBDDToBase(fromHome)));
		}

		/*
		 * if(id == 1){ System.out.println("reading:\n " +
		 * Env.toNiceString(main, adjustBDDToBase(reading)));
		 * System.out.println("fromHome:\n " + Env.toNiceString(main,
		 * adjustBDDToBase(fromHome)));
		 * System.out.println("othersGlobalTrans from home:\n " +
		 * Env.toNiceString(main, adjustBDDToBase(othersGlobalTrans))); }
		 */
		for (int i = 1; i < N; i++) {
			if (i != id) {
				BDD temp = adjustBDDToManager(otherThreads[i].outputChannels[id]
						.read());
				if (id == 1) {
					System.out.println("  ------>  id=1 got temp from " + i
							+ ":\n "
							+ Env.toNiceString(main, adjustBDDToBase(temp)));
				}
				othersGlobalTrans = othersGlobalTrans.or(temp
						.and(idleTransForP));
			}
		}
	...
	public BDD getNewThetaConjunct() {
		return adjustBDDToManager(newTheta);
	}
...
/*
 * //an output channel of bdds and markers public class Channel { private int
 * manIdx; private ConcurrentLinkedQueue<Object> queue;
 * 
 * public Channel(JTLVThread thread) { this.manIdx =
 * Env.assignDedicatedBDDManagerIdx(thread); queue = new
 * ConcurrentLinkedQueue<Object>(); }
 * 
 * public BDD read() { return (BDD)queue.poll(); }
 * 
 * public void writeBDD(BDD to_write) { queue.add(Env.copyBDDIntoManager(manIdx,
 * to_write)); }
 * 
 * }
 */