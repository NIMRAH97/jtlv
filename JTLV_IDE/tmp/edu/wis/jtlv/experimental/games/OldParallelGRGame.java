 ...
 * {@link OldParallelGRGame#calculate_strategy()}.
 * </p>
 * 
 ...
 * 
 */
public class OldParallelGRGame implements TwoPlayersGame {
	 ...
	 */
	public OldParallelGRGame(ModuleWithWeakFairness env, ModuleWithWeakFairness sys)
			throws GameException {
		...
		for (iterZ = new FixPoint<BDD>(); iterZ.advance(z);) {
			for (int j = 0; j < sys.justiceNum(); j++) {
				...
				for (iterY = new FixPoint<BDD>(); iterY.advance(y);) {
					...
					int justice_num = env.justiceNum();
					allNu = new InnerX[10];
					for (int ii = 0; (10 * ii) < justice_num; ii++) {
						for (int i = 0; i < 10; i++) {
							if (((10 * ii) + i) >= justice_num)
								continue;
							allNu[i] = new InnerX(j, (ii * 10) + i, cy, z,
									start);
						}
						for (int i = 0; i < 10; i++) {
							if (((10 * ii) + i) >= justice_num)
								continue;
							allNu[i].start();
							// runIt(j, (ii * 10) + i, cy, z, start);
						}
						for (int i = 0; i < 10; i++) {
							if (((10 * ii) + i) >= justice_num)
								continue;
							try {
								// when the thread joins, we calculate y
								allNu[i].join();
