 ...
 * 
 */
public class GDRGame extends JTLVThread implements TwoPlayersGame {
	private ModuleWithWeakFairness env;
	private ModuleWithWeakFairness sys;
	private BDD[] sys_justices;
	private BDD env_single_justice;
	private BDD sys_trans;
	private BDD env_trans;
	private BDDVarSet sys_prime_vars;
	private BDDVarSet env_prime_vars;

	private BDD player2_winning;

	private int env_focus;

	// p2_winning in GRGAmes are !p1_winning

	public GDRGame(ModuleWithWeakFairness env, int env_focus,
			ModuleWithWeakFairness sys) throws GameException {
		if ((env == null) || (sys == null)) {
			throw new GameException(
					"cannot instanciate a GR[1] Game with an empty player.");
		}

		this.env = env; // to be used only for extracting the strategy!!!
		this.sys = sys; // to be used only for extracting the strategy!!!

		this.sys_justices = super.adjustBDDToManager(sys.allJustice());
		this.env_focus = env_focus;
		this.env_single_justice = super.adjustBDDToManager(env
				.justiceAt(env_focus));
		this.sys_trans = super.adjustBDDToManager(sys.trans());
		this.env_trans = super.adjustBDDToManager(env.trans());
		this.sys_prime_vars = super.adjustBDDVarSetToManager(sys
				.modulePrimeVars());
		this.env_prime_vars = super.adjustBDDVarSetToManager(env
				.modulePrimeVars());

		// // for now I'm giving max_y 50, at the end I'll cut it (and during,
		// I'll
		// // extend if needed. (using vectors only makes things more complicate
		// // since we cannot instantiate vectors with new vectors)
		// x_mem = new BDD[sys.justiceNum()][1][50];
	...
	public void run() {
		...
		// extend if needed. (using vectors only makes things more complicate
		// since we cannot instantiate vectors with new vectors)
		x_mem = new BDD[sys_justices.length][1][50];
		y_mem = new BDD[sys_justices.length][50];
		z_mem = new BDD[sys_justices.length];
		this.player2_winning = this.calculate_win();
	}
	...
	private BDD calculate_win() {
		...
		for (iterZ = new FixPoint<BDD>(); iterZ.advance(z);) {
			for (int j = 0; j < sys_justices.length; j++) {
				cy = 0;
				y = Env.FALSE();
				for (iterY = new FixPoint<BDD>(); iterY.advance(y);) {
					BDD start = sys_justices[j].and(cox(z)).or(cox(y));
					y = Env.FALSE();
					BDD negp = env_single_justice.not();
					x = z.id();
					for (iterX = new FixPoint<BDD>(); iterX.advance(x);) {
						x = negp.and(cox(x)).or(start);
					}
					x_mem[j][0][cy] = x.id();
					y = y.id().or(x);
					y_mem[j][cy] = y.id();
	...
	public BDD cox(BDD to) {
		BDDVarSet responder_prime = sys_prime_vars;
		BDDVarSet this_prime = env_prime_vars;
		BDD exy = Env.prime(to).and(sys_trans).exist(responder_prime);
		return env_trans.imp(exy).forAll(this_prime);
	}
	...
	public void calculate_strategy(int kind) {
		...
		// converting the results back to base manager...
		z_mem = super.adjustBDDToBase(z_mem);
		for (int i = 0; i < y_mem.length; i++) {
			y_mem[i] = super.adjustBDDToBase(y_mem[i]);
		}
		for (int i = 0; i < x_mem.length; i++) {
			for (int j = 0; j < x_mem[i].length; j++) {
				x_mem[i][j] = super.adjustBDDToBase(x_mem[i][j]);
			}
		}

		BDD all_init = env.initial().and(sys.initial());
		BDDVarSet all_unprime = env.moduleUnprimeVars().union(
				sys.moduleUnprimeVars());

		// initiating the stacks
		for (BDDIterator all_states = all_init.iterator(all_unprime); all_states
				.hasNext();) {
			...
			for (Iterator<BDD> iter_succ = succs.iterator(); iter_succ
				...
				while (candidate.isZero() & (local_kind >= 0)) {
					...
					if ((local_kind == 1) | (local_kind == 6)
						...
						if (!p_st.and(env.justiceAt(env_focus).not()).isZero()) {
							BDD opt = next_op.and(x_mem[p_j][0][p_cy]);
							if (!opt.isZero()) {
								candidate = opt;
							}
						}
		...
		System.out.print("\n\n");
		System.out.print("results base of faling environment justice "
				+ env_focus + "\n");
		System.out.print("=============================================\n");
	...
	public BDD sysWinningStates() {
		return super.adjustBDDToBase(player2_winning);
	}
	...
	public BDD envWinningStates() {
		return super.adjustBDDToBase(player2_winning.not());
	}

	public BDD gameInitials() { // already in base...
