...
import edu.wis.jtlv.lib.games.Game;
 ...
 * 
 */
public class GRGame implements Game {
	public SMVModule env;
	public SMVModule sys;
	private BDD player2_winning;

	// p2_winning in GRGAmes are !p1_winning

	public GRGame(SMVModule env, SMVModule sys) throws IllegalGameException {
		...
		x_mem = new BDD[sys.justice_num()][env.justice_num()][50];
		y_mem = new BDD[sys.justice_num()][50];
		z_mem = new BDD[sys.justice_num()];
	...
	public BDD calculate_win() {
		...
		z = Env.TRUE();
		for (FixPoint<BDD> iter_z = new FixPoint<BDD>(); iter_z.advance(z);) {
			for (int j = 0; j < sys.justice_num(); j++) {
				cy = 0;
				y = Env.FALSE();
				for (FixPoint<BDD> iter_y = new FixPoint<BDD>(); iter_y
						.advance(y);) {
					BDD start = sys.justice(j).and(cox(z)).or(cox(y));
					y = Env.FALSE();
					for (int i = 0; i < env.justice_num(); i++) {
						BDD negp = env.justice(i).not();
						x = z.id();
						for (FixPoint<BDD> iter_x = new FixPoint<BDD>(); iter_x
								.advance(x);) {
	...
	private BDD cox(BDD to) {
		BDD prime_to = Env.prime(to);
		BDD exy = prime_to.and(sys.trans()).exist(sys.prime_vars());
		BDD unprime_res = env.trans().imp(exy).forAll(env.prime_vars());
		// return Env.prime(prime_res);
		return unprime_res;
	}
	...
	public void calculate_strategy(int kind) {
		...
		for (BDDIterator all_states = all_init.iterator(env.unprime_vars()
				.union(sys.unprime_vars())); all_states.hasNext();) {
		...
		while (!st_stack.isEmpty()) {
			...
			for (BDDIterator all_states = all_succs
					.iterator(env.unprime_vars()); all_states.hasNext();) {
				BDD sin = (BDD) all_states.next();
				succs.add(sin);
			}
			...
			for (Iterator<BDD> iter_succ = succs.iterator(); iter_succ
				...
				BDD next_op = Env.unprime(sys.trans().and(p_st).and(
						primed_cur_succ).exist(
						env.unprime_vars().union(sys.unprime_vars())));
				...
				while (candidate.isZero() & (local_kind >= 0)) {
					...
					if ((local_kind == 3) | (local_kind == 7)
						...
						int next_p_j = (p_j + 1) % sys.justice_num();
						if (!p_st.and(sys.justice(p_j)).isZero()) {
					...
					if ((local_kind == 1) | (local_kind == 6)
						...
						if (!p_st.and(env.justice(p_i).not()).isZero()) {
				...
				BDD one_cand = candidate.satOne(env.unprime_vars().union(
						sys.unprime_vars()), false);
	...
	public SMVModule get_env_player() {
		return env;
	}

	public SMVModule get_sys_player() {
		return sys;
	}

	public BDD sys_winning_states() {
		return player2_winning;
	}

	public BDD env_winning_states() {
